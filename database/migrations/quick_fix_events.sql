-- Quick Fix for Events Schema - No Foreign Key Dependencies
-- This will make the events system work immediately
...
ADD COLUMN IF NOT EXISTS registration_count bigint DEFAULT 0;

-- 2. Remove old payment columns if they exist
ALTER TABLE public.events DROP COLUMN IF EXISTS ticket_price;
ALTER TABLE public.events DROP COLUMN IF EXISTS ticket_url;

-- 3. Update event_tickets table
ALTER TABLE public.event_tickets 
ADD COLUMN IF NOT EXISTS registered_quantity bigint DEFAULT 0;
...
-- 5. Create storage policies for event images
...
-- 6. Set up universal RLS policies
...
-- 9. Insert default event categories if they don't exist
